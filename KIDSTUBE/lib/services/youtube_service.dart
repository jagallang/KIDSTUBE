...
class YouTubeService implements IYouTubeService {
  ...
  Future<bool> validateApiKey() async {
    // 테스트 키 허용
    if (apiKey == 'TEST_API_KEY') {
      print('🧪 테스트 API 키 사용 - 검증 통과');
      return true;
    }
    
    // API 키 형식 검증 (AIza로 시작하는 39자리)
    if (!apiKey.startsWith('AIza') || apiKey.length != 39) {
      print('❌ 잘못된 API 키 형식: ${apiKey.length}자리, AIza로 시작해야 함');
      return false;
    }
    
    try {
      print('🔍 API 키 검증 중: ${apiKey.substring(0, 8)}...');
      ...
      print('📊 검증 응답 상태: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        print('❌ API 키 검증 실패: ${response.statusCode}');
        print('📄 응답 내용: $responseBody');
        
        if (response.statusCode == 400) {
          print('🔑 400 오류: API 키가 유효하지 않습니다');
        } else if (response.statusCode == 403) {
          if (responseBody.contains('quotaExceeded')) {
            print('📊 403 오류: API 할당량 초과');
          } else if (responseBody.contains('keyInvalid')) {
            print('🔑 403 오류: 잘못된 API 키');
          } else {
            print('🚫 403 오류: API 사용 권한 없음');
          }
        }
        return false;
      }
    } catch (e) {
      print('⚠️ API 검증 예외 발생: $e');
      return false;
    }
  }
